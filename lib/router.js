...
Router.prototype.handleSuccess = function(data, optionsOrCallback, callback) {
  ...
  if (typeof optionsOrCallback === 'function') {
    ...
    if(optionsOrCallback.modelName){
      modelName = optionsOrCallback.modelName;
    }
    if(optionsOrCallback.eventType){
      eventType = optionsOrCallback.eventType;
    }
  }
  ...
  if (!!daoFactory) {
    daoFactory
      ...
      .success(function(entries) {
        ...
        this.handleSuccess(entries,  {modelName: modelName, eventType: 'query'}, callback)
      }.bind(this))
      .error(function(err) {
    ...
    daoFactory
      ...
      .success(function(entry) {
        if (!entry){
          ...
          this.handleSuccess(entry.values,  {modelName: modelName, eventType: 'get'}, callback)
        }
      }.bind(this))
      .error(function(err) {
        this.handleError(err, callback)
...
var handleResourceCreate = function(modelName, attributes, callback) {
  ...
  if (!!daoFactory) {
    daoFactory
      ...
      .success(function(entry) {
        if (!entry){
          this.handleSuccess({}, {modelName: modelName, eventType: 'create'}, callback)
        }else{
          this.handleSuccess(entry.values, {modelName: modelName, eventType: 'create'}, callback)
        }
      }.bind(this))
      .error(function(err) {
        this.handleError(err, callback)
...
var handleResourceUpdate = function(modelName, identifier, attributes, callback) {
  ...
  if (!!daoFactory) {
    daoFactory
      ...
      .success(function(entry) {
        if (!entry){
          // if object doesn't exists create new
          daoFactory.create(attributes).complete(function(err, entry){
            if (err){
              this.handleError(err, callback);
            } else {
              this.handleSuccess(entry.values, {modelName: modelName, eventType: 'create'}, callback);
          ...
          entry.updateAttributes(attributes).complete(function(err, entry) {
            if (err) {
              ...
              this.handleSuccess(entry.values, {modelName: modelName, eventType: 'update'}, callback)
            }
          }.bind(this))
        }
      }.bind(this))
          ...
          entry
            ...
            .complete(function(err) {
              if (err) {
                ...
                this.handleSuccess({id:entry.id}, {modelName: modelName, eventType: 'delete'}, callback)
              }
            }.bind(this))
        }
      }.bind(this))
